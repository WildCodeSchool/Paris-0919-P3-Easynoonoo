...
export default class FamilyAgenda extends React.Component {
  ...
  findWeekDay = date => {
    /*To convert moment date -> day, array can be customized */
    ...
    if (this.state.items.length > 0) {
      ...
      if (items === null) {
        ...
        if (this.state.items[0] != this.state.items[1]) {
          ...
          if (dayStart == endStart) {
            // if the same day was selected in start and end
            items.push({
  ...
  addChild = () => {
    ...
    let arrayChildren = JSON.parse(
      localStorage.getItem('allChildren'),
    )
    ...
    if (i < myChild.length) {
      //look at the size of my Children array

      if (items.length > 0) {
        // if the user selected some timeslot
        nameChild = [...nameChild, myChild[i]]

        for (let k = 0; k < items.length; k++) {
          // loop to generate one info object per time slot per child
          let arrayTr = []
          let objChild = {}
  ...
  updateChildName = () => {
    ...
    if (
      this.state.calendarChild == undefined &&
      countMyChild == myChild.length
    ) {
      // && countMyChild - countNotMyChild == countMyChild
  ...
  sendData = () => {
    let timeSlotObject = JSON.parse(
      localStorage.getItem('allChildren'),
    )

    axios
      .post(
        'http://localhost:4000/api/calculRepartition',
        timeSlotObject,
      ) //POST - POST => envoyer infos
      .then(res => {
  ...
  render() {
    ...
    return (
      <>
                    ...
                    return (
                      <>
                        ...
                        {hours.map(hour => {
                          return (
                            <tr
                              className="agenda__row_hour-start"
                              draggable="false"
                            >
                              {columns.map(column => {
                                return (
                    ...
                    return (
                      <>
                        ...
                        {hours.map(hour => {
                          return (
                            <tr
                              className="agenda__row_hour-startBis"
                              draggable="false"
                            >
                              {columns.map(column => {
                                return (
        ...
        <h1 className="h1Name">
          Planning pour {this.state.calendarChild}
        </h1>
        ...
        <div class="input-group">
          <select
            ...
            value={this.state.valueOnClick}
            onChange={this.handleResetPlanning}
          >
            <option selected value="null">
              --Merci de choisir une option--
            </option>
            <option selected value="oui">
